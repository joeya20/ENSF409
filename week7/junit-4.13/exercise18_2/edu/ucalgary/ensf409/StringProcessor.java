...
public class StringProcessor{
    ...
    public static void main(String[] args) {
        var test = new StringProcessor("z");
        System.out.println(StringProcessor.idProcessing("J-a", "Ah", "Buddy", 2010));
        System.out.println(test.secretCode(1));
    }
    ...
    public static String idProcessing(String firstName, String lastName, String petName, int year) {
        ...
        if((lastName.length() > 26) || !nameMatcher.matches()){
            throw new IllegalArgumentException();
        }
        //check pet name
        nameMatcher = namePattern.matcher(petName);
        puncMatcher = puncPattern.matcher(petName);
        if((petName.length() > 26) || !nameMatcher.matches()){
            throw new IllegalArgumentException();
        }
